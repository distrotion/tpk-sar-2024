...
import 'package:tpk_login_arsa_01/page/12SummaryDataPage/SubTAB/4TestPieceCount/SubWidget/TestPieceCountTable/TestPieceCountTable.dart';
...
late BuildContext TestPieceCountContext;

class TestPieceCount extends StatelessWidget {
  const TestPieceCount({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<ManageDataTestPieceCount>(
          create: (BuildContext context) => ManageDataTestPieceCount(),
        ),
      ],
      child: SingleChildScrollView(
        child: Container(
          margin: EdgeInsets.only(left: 50, right: 50, top: 20),
          child: TestPieceCountWidget(),
        ),
      ),
    );
  }
...
class TestPieceCountWidget extends StatefulWidget {
  const TestPieceCountWidget({Key? key}) : super(key: key);

  @override
  _TestPieceCountWidgetState createState() => _TestPieceCountWidgetState();
}

class _TestPieceCountWidgetState extends State<TestPieceCountWidget> {
  void initState() {
    print("InINITIAL");
    super.initState();
  }
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          BlocBuilder<ManageDataTestPieceCount, int>(builder: (context, state) {
            print("TestPieceCountWidget rebuild state :$state");
            if (state >= 0 && state < 3)
              return Column(
                children: [
                  SizedBox(
...
class _SearchBarState extends State<SearchBar> {
  ...
  void clearSearch() {
    print('inclear');
    searchOption.clear();
    searchOption.add(SearchTestPieceCountModel());
