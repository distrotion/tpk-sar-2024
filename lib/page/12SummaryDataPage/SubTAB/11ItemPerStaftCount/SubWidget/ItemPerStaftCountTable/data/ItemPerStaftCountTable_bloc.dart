...
class ManageDataItemPerStaftCount extends Bloc<ItemPerStaftCountEvent, int> {
  ManageDataItemPerStaftCount() : super(0);

  @override
  Stream<int> mapEventToState(ItemPerStaftCountEvent event) async* {
    if (event == ItemPerStaftCountEvent.clearAndSearch) {
      yield* clearAndSearch();
    } else if (event == ItemPerStaftCountEvent.searchKPIdata) {
      yield* searchKPIData();
    }
  }
}
...
Stream<int> clearAndSearch() async* {
  yield 0;
  ItemPerStaftCountContext.read<ManageDataItemPerStaftCount>()
      .add(ItemPerStaftCountEvent.searchKPIdata);
}

List<SearchItemPerStaftModel> searchOption = [];
List<ItemPerStaftCountModel> dataTableItemPerStaft = [];
Stream<int> searchKPIData() async* {
  Map<String, String> qParams = {
    'SearchOption': jsonEncode(searchOption),
  };
  EasyLoading.show(status: 'loading...');
  print("in /SummaryDataPage/ItemPerStaftCount_searchItemPerStaftData");
  try {
    final response = await http
        .post(
            Uri.parse(
                "$urlE/SummaryDataPage/ItemPerStaftCount_searchItemPerStaftData"),
            body: qParams)
    ...
    if (response.statusCode == 200) {
      if (response.body != "error") {
        ...
        if (response.body != "[]") {
          EasyLoading.dismiss();
          dataTableItemPerStaft = ItemPerStaftCountModelFromJson(response.body);
          print(dataTableItemPerStaft.length.toString());
          print(dataTableItemPerStaft[0].instrumentName);
          yield 2;
        } else {
          EasyLoading.dismiss();
...
Future searchMasterOption() async {
  ...
  try {
    final response = await http
        .post(Uri.parse(
            "$urlE/SummaryDataPage/ItemPerStaftCount_searchMasterOption"))
        .timeout(Duration(seconds: timeOut));
