...
import 'package:tpk_login_arsa_01/page/12SummaryDataPage/SubTAB/2ItemRecheckCount/SubWidget/ItemRecheckCountTable/ItemRecheckCountTable.dart';

late BuildContext contextItemRecheckCount;

class ItemRecheckCount extends StatelessWidget {
  const ItemRecheckCount({Key? key}) : super(key: key);
            ...
            child: Container(
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      Container(width: 1100, child: ItemRecheckCountTable()),
                    ],
                  ),
                ))));
  }
...
import 'package:tpk_login_arsa_01/page/12SummaryDataPage/SubTAB/6ItemRecheckCount/SubWidget/ItemRecheckCountTable/ItemRecheckCountTable.dart';
...
late BuildContext ItemRecheckCountContext;
...
class ItemRecheckCount extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<ManageDataItemRecheckCount>(
          create: (BuildContext context) => ManageDataItemRecheckCount(),
        ),
      ],
      child: SingleChildScrollView(
        child: Container(
          margin: EdgeInsets.only(left: 50, right: 50, top: 20),
          child: ItemRecheckCountWidget(),
        ),
      ),
    );
  }
...
class ItemRecheckCountWidget extends StatefulWidget {
  const ItemRecheckCountWidget({Key? key}) : super(key: key);

  @override
  _ItemRecheckCountWidgetState createState() => _ItemRecheckCountWidgetState();
}

class _ItemRecheckCountWidgetState extends State<ItemRecheckCountWidget> {
  void initState() {
    print("InINITIAL");
/*     context
        .read<ManageDataItemRecheckCount>()
  ...
  @override
  Widget build(BuildContext context) {
    ItemRecheckCountContext = context;
    return Form(
      key: _formKey,
      child: Column(
        ...
        children: [
          ...
          BlocBuilder<ManageDataItemRecheckCount, int>(builder: (context, state) {
            print("ItemRecheckCountWidget rebuild state :$state");
            if (state >= 0 && state < 3)
              return Column(
                children: [
                  SizedBox(
...
class _SearchBarState extends State<SearchBar> {
  ...
  void clearSearch() {
    print('inclear');
    searchOption.clear();
    searchOption.add(SearchItemRecheckModel());
