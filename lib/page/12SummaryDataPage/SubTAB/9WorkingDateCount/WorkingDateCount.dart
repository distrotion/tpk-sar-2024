...
import 'package:tpk_login_arsa_01/page/12SummaryDataPage/SubTAB/9WorkingDateCount/SubWidget/WorkingDateCountTable/WorkingDateCountTable.dart';
...
late BuildContext WorkingDateCountContext;

class WorkingDateCount extends StatelessWidget {
  const WorkingDateCount({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<ManageDataWorkingDateCount>(
          create: (BuildContext context) => ManageDataWorkingDateCount(),
        ),
      ],
      child: SingleChildScrollView(
        child: Container(
          margin: EdgeInsets.only(left: 50, right: 50, top: 20),
          child: WorkingDateCountWidget(),
        ),
      ),
    );
  }
...
class WorkingDateCountWidget extends StatefulWidget {
  const WorkingDateCountWidget({Key? key}) : super(key: key);

  @override
  _WorkingDateCountWidgetState createState() => _WorkingDateCountWidgetState();
}

class _WorkingDateCountWidgetState extends State<WorkingDateCountWidget> {
  void initState() {
    print("InINITIAL");
/*     context
        .read<ManageDataWorkingDateCount>()
  ...
  @override
  Widget build(BuildContext context) {
    WorkingDateCountContext = context;
    return Form(
      key: _formKey,
      child: Column(
        ...
        children: [
          ...
          BlocBuilder<ManageDataWorkingDateCount, int>(
              builder: (context, state) {
            print("WorkingDateCountWidget rebuild state :$state");
            if (state >= 0 && state < 3)
              return Column(
                children: [
                  SizedBox(
...
class _SearchBarState extends State<SearchBar> {
  ...
  void clearSearch() {
    print('inclear');
    searchOption.clear();
    searchOption.add(SearchWorkingDateCountModel());
                              ...
                              child: IconButton(
                                ...
                                onPressed: () {
                                  context
                                      .read<ManageDataWorkingDateCount>()
                                      .add(
                                          WorkingDateCountEvent.clearAndSearch);
                                },
                              ),
                            ),
                            Container(
