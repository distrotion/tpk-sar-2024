...
class ManageDataRequestCount extends Bloc<RequestCountEvent, int> {
  ManageDataRequestCount() : super(0);

  @override
  Stream<int> mapEventToState(RequestCountEvent event) async* {
    if (event == RequestCountEvent.clearAndSearch) {
      yield* clearAndSearch();
    } else if (event == RequestCountEvent.searchKPIdata) {
      yield* searchKPIData();
    }
  }
}
...
Stream<int> clearAndSearch() async* {
  yield 0;
  RequestCountContext.read<ManageDataRequestCount>()
      .add(RequestCountEvent.searchKPIdata);
}

List<SearchRequestCountModel> searchOption = [];
List<RequestCountModel> dataTableRequestCount = [];
Stream<int> searchKPIData() async* {
  Map<String, String> qParams = {
    'SearchOption': jsonEncode(searchOption),
  };
  EasyLoading.show(status: 'loading...');
  print("in /SummaryDataPage/RequestCount_searchTestPieceData");
  try {
    final response = await http
        .post(Uri.parse("$urlE/SummaryDataPage/RequestCount_searchRequestData"),
            body: qParams)
    ...
    if (response.statusCode == 200) {
      if (response.body != "error") {
        ...
        if (response.body != "[]") {
          EasyLoading.dismiss();
          dataTableRequestCount = RequestCountModelFromJson(response.body);
          print(dataTableRequestCount.length.toString());
          yield 2;
        } else {
          EasyLoading.dismiss();
...
Future searchMasterOption() async {
  ...
  try {
    final response = await http
        .post(
            Uri.parse("$urlE/SummaryDataPage/RequestCount_searchMasterOption"))
        .timeout(Duration(seconds: timeOut));
