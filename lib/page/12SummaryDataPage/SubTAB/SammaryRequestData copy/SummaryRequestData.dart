...
import 'package:tpk_login_arsa_01/page/12SummaryDataPage/SubWidget/SammaryRequestData%20copy/data/SummaryRequestData_bloc.dart';
...
class SummaryRequestData extends StatefulWidget {
  String branch = "";
  SummaryRequestData({Key? key, required this.branch}) : super(key: key);

  @override
  State<StatefulWidget> createState() => SummaryRequestDataState();
}

class SummaryRequestDataState extends State<SummaryRequestData> {
  ...
  void initState() {
    ...
    if (widget.branch == "BANGPOO") {
      context
          .read<ManageDataSummaryRequestData>()
          .add(SummaryRequestDataEvent.fetchSummaryRequestDataBP);
    } else {
      context
          .read<ManageDataSummaryRequestData>()
          .add(SummaryRequestDataEvent.fetchSummaryRequestDataRY);
    }
  ...
  List<ModelSummaryRequestData> summaryItemData = [];

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        BlocBuilder<ManageDataSummaryRequestData, int>(builder: (context, state) {
          if (state == 0) {
            return Container();
          } else {
            if (widget.branch == "BANGPOO") {
...
/* class SummaryRequestData2 extends StatelessWidget {
  const SummaryRequestData2({Key? key}) : super(key: key);
